...
test('quand l‘utilisateur appelle increment, fait les bons appels', async ({ page }) => {
	let callCount = 0;
...
test('quand l‘utilisateur appelle increment et qu‘il y a une erreur, affiche l‘erreur', async ({ page }) => {
	let callCount = 0;
...
test('quand l‘utilisateur appelle increment et qu‘elle est en succès, affiche un message de succès', async ({ page }) => {
	await page.route('https://rpc-amoy.polygon.technology/', async route => {
		await route.fulfill({ json: { jsonrpc: "2.0", id: 0, result: "0x" } });
	});
